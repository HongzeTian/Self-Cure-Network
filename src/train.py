...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default=r'D:\thz\data\RAF-DB', help='Raf-DB dataset path.')
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Pytorch checkpoint file path')
    # parser.add_argument('--pretrained', type=str, default='../models/epoch35_acc0.8713.pth',
    #                     help='Pretrained weights')
    parser.add_argument('--pretrained', type=str, default='../pretrained/ijba_res18_naive.pth.tar',
                        help='Pretrained weights')
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, transform = None, basic_aug = False):
        ...
        NAME_COLUMN = 0
        LABEL_COLUMN = 1
        df = pd.read_csv(os.path.join(self.raf_path, 'EmoLabel/list_patition_label_balanced.txt'), sep=' ', header=None)
        if phase == 'train':
        ...
        for f in file_names:
            f = f.split(".")[0]
            f = f +"_aligned.jpg"
            path = os.path.join(self.raf_path, 'Image/combined_Tsinghua', f)
...
def run_training():
    ...
    if args.pretrained:
        ...
        pretrained_state_dict = pretrained['state_dict'] # this is for ijba_res18_naive
        # pretrained_state_dict = pretrained['model_state_dict'] # this is for trained weight
        ...
        with torch.no_grad():
            ...
            if acc > 0.865 :
                torch.save({'iter': i,
                            ...
                            os.path.join('../with_tqinghua_models_new_labels', "epoch"+str(i)+"_acc"+str(acc)+".pth"))
                print('Model saved.')
